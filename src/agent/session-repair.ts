/**
 * Session file (JSONL) repair.
 * Adapted from OpenClaw's session-file-repair.ts.
 *
 * Detects corrupted JSONL lines (e.g. from a crash mid-write),
 * drops them, backs up the original, and writes back a clean file.
 */
...
/**
 * Repair a JSONL session file if it contains malformed lines.
 * - Reads the file line by line
 * - Drops lines that aren't valid JSON
 * - If any were dropped: backs up original, writes cleaned version
 * - If the first entry isn't a session header, skips repair (file is too broken)
 */
